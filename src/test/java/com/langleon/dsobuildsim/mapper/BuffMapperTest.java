...
public class BuffMapperTest {
    ...
    {
        BuffDTO buffDTO = BuffMapper.from(buffConfig.tonics().get(TonicType.VIGOR));

        Assertions.assertEquals("VIGOR", buffDTO.type());
        Assertions.assertEquals("Tonic of Vigor", buffDTO.name());
        Assertions.assertEquals(5, buffDTO.tier());
    ...
    {
        BuffDTO buffDTO = BuffMapper.from(buffConfig.physics().get(PhysicType.VIGOR_ADVANCED));
